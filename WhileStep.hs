...
evalS stmt = 
  case stmt of
    ...
    Throw expr _      -> do
      ...
      throwError val
    Try s1 var s2 _   -> do
      return (Just s1) `catchError` (\val -> return $ Just (Sequence (Assign var (Val val) (-1)) s2))
...
breakOn :: (MonadState Store m, MonadError Value m, MonadWriter String m) => [Int] -> Statement -> m (Maybe Statement)
breakOn bps stmt = do
  next <- evalS stmt
  case next of
    Nothing    -> return Nothing
...
run :: [Int] -> Store -> Statement -> (Store, Maybe Value, String, Maybe Statement)
